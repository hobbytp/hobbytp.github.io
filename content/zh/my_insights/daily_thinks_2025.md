...
### 2025-11-21
今天看了“Ryan硅谷创业”小红书的一个帖子，讨论大模型最新进展，有些不同或类似个人观点：

1. 微调地位确实下降了，大模型出太快，刚微调完，又要重调了，而且模型绑定，确实没有Agentic Context Engineering（ACE）香，毕竟上层（ACE）优化是可以继承和灵活演进的，与模型解耦。微调更多是用于小模型（如 7B/8B 级别）的特定任务蒸馏，为了端侧部署或极致cost/latency优化。
2. 大企业选开源，看今年美国逐渐有知名企业开始用中国模型了。但是美国企业大多还是会用闭源的吧，数据上签一个private协议，只是混用会越来越成常态。成本和不愿被绑定都是要考虑的。别的国家如果不是要考虑地缘政治，开源大模型会是首选，套个壳做闭源也不是不可能。
3. 另外学术机构也偏爱开源，可复现性，魔改需求，闭源都给不了。
4. 个人更多是双选, 比如编码会用多套：cursor 和 Claude Code(+ GLM，M2，K2)，还是蛮爽的，高阶用户改造成日常多智能体自动代理，国产开源的高性价比更是舍我其谁。App也是用多套（gemini, chatgpt,豆包，deepseek，元宝，千问）。
5. 大模型底座，一方面不会放弃transformer演进，毕竟这个目前还是稳妥些，更多维度的scaling law还没有探索完，配合ACE，可以不改底座的情况下再持续发展一会。 但是一定会做其他方面的探索，毕竟transformer的硬伤也在那。
...
宾夕法尼亚大学研究发现（论文《[Mind Your Tone](https://arxiv.org/pdf/2510.04950)》）, 向GPT-4o等新一代大语言模型使用简洁直接或略带粗鲁的提问，可减少冗余修饰语，聚焦核心问题，从而将多项选择题准确率从约80.8%提升至84.8%；但该结论仅基于多选题实验，且不同模型对语气敏感度各异，应用时需在效率与礼貌间权衡。
