...
目前支持在线文档、PDF、GitHub仓库三种类型的技能， 可以通过下面的两种方式来生成对应的技能。
>注意： 要提供GITHUB_TOKEN， 否则会容易触发github的api ratelimit。
...
2. 搭建环境：
   ...
   `uv venv .venv --python 3.12` 创建虚拟环境并安装依赖。  
3. 一键安装：运行 `./setup_mcp.sh` 设置环境。  
4. 示例生成 React 技能：使用一个config json文件来包含相关的资源信息（文档，pdf，github仓库）
   a. 文档爬取：`uv run python cli/doc_scraper.py --config configs/react.json`  
   b. PDF 支持：增加 `--pdf` 参数支持表格及 OCR 提取。  
   c. GitHub 爬取：`uv run python cli/github_scraper.py --repo facebook/react`。  
   d. 多源整合爬取与冲突检测：`uv run python cli/unified_scraper.py --config configs/myframework_unified.json`。  
5. 打包并上传：`uv run python cli/package_skill.py output/react/` 上传至 Claude AI 即完成技能创建。  

**case： Hugo 文档**

```bash
uv run python cli/github_scraper.py --config configs/hugo.json
```
...
```json
{
  ...
  "description": "Hogo Documentation",
...
```bash
uv run python cli/package_skill.py output/hugo/
```

将在./output/hugo.zip中生成技能zip文件。把hugo.zip文件解压到.claude/skills目录下,重启claude code即可。

#### 方式二：使用Claude Code来生成skill zip
...
2. 搭建环境：
   ...
   `uv venv .venv --python 3.12` 创建虚拟环境并安装依赖。  
3. 一键安装：运行 `./setup_mcp.sh` 设置环境。  
3. 进入claude code，运行下面的prompt，claude code会帮助完成剩下的事情。

##### Prompt例子

在Skill-seekers目录下，打开claude code， 键入下面的prompt cases:

- case 1: 只有在线文档

```bash
Generate a claude-skills skill from https://docs.claude.com/en/docs/agents-and-tools/agent-skills/ and https://github.com/anthropics/claude-cookbooks/tree/main/skills

```

- case 2：PDF文档和Github 仓库
  
```bash
为我的项目创建一个综合技能，结合：
- PDF 手册：./pdf/2510-ACE.pdf
- GitHub 仓库：suzgunmirac/dynamic-cheatsheet

配置名称：ace
合并模式：rule-based
```

- case 3：在线文档，PDF文档和Github 仓库

```bash
为我的项目创建一个综合技能，结合：
- 在线文档：https://huggingface.co/Alibaba-NLP/Tongyi-DeepResearch-30B-A3B 和 https://tongyi-agent.github.io/blog/introducing-tongyi-deep-research/
- PDF 手册：./pdf/2510-tongyi-deepresearch-tech-report.pdf
- GitHub 仓库：Alibaba-NLP/DeepResearch

配置名称：tongyi_deepresearch
合并模式：rule-based
```

```bash
Generate a cc_infro skill from
- online doc：https://www.reddit.com/r/ClaudeAI/comments/1oivjvm/claude_code_is_a_beast_tips_from_6_months_of/
- GitHub repo：diet103/claude-code-infrastructure-showcase

config name：cc_infro
